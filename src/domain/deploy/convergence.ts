 ...
 *
 * A rolling deploy touches replicas one at a time, so an interrupted roll
 ...
 * reveals it — `status` has to compare them.
 */
...
export function assessConvergence(observations: ReplicaObservation[]): FleetConvergence {
  ...
  return {
    ...
    converged: releases.length <= 1 && undeployed.length === 0,
  };
}
