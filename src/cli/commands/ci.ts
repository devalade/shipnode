  ...
  const envSyncSteps = options.syncEnv && envFile && options.app
    ? (() => {
      ...
      - name: Materialize application environment
        env:
          SHIPNODE_CI_SECRET_FILE: \${{ secrets.${secretName} }}
        run: |
          [ -n "$SHIPNODE_CI_SECRET_FILE" ] || { echo "Missing GitHub Environment secret: ${secretName}" >&2; exit 1; }
          umask 077
          mkdir -p "$(dirname ${quotedEnvFile})"
          printf '%s' "$SHIPNODE_CI_SECRET_FILE" > ${quotedEnvFile}${stepWorkingDirectory}
