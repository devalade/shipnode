...
- **`.servers({ user, hosts })` replaces the named-servers record.** The host string is the server's identity everywhere — `on`, `--on`, accessory `on`, status output — and the order of `hosts` is deployment order. `user` is the SSH user for every host without its own `user@` prefix (default `root`). The old record form still parses but is undocumented.
...
- **`.fleet()`, `.group()`, `drainWait`, `readyPath` are removed.** An `on` target resolving to more than one host is a fleet — rolled one replica at a time, blue-green enabled automatically for fleet backends — and `.group()` is replaced by naming hosts directly in `on`.
- **`shipnode drain` / `shipnode undrain` are removed.** Fleet replicas are no longer drained during a roll (see ADR-0008): blue-green keeps a replica serving until the new release passes health, so there is nothing to drain.

### Documentation

- **New [load balancer guide](https://shipnode.dev/docs/load-balancer)** — the one-page fleet/LB contract (targets, health check, TLS), the once-only verification loop, and what a failed roll looks like. The README fleet section links it.

### Added
- **Fleet mode — one app on N servers behind your load balancer.** An `on` target resolving to more than one host makes the app a fleet: `shipnode deploy` rolls through its replicas one at a time. Blue-green stays intra-replica — a replica keeps serving its old release until the new one passes health — so the composition is blue-green within a replica, rolling across replicas. See [ADR-0007](docs/adr/0007-fleet-replication.md) and [ADR-0008](docs/adr/0008-reactive-health.md).
  - **Shipnode does not manage the load balancer.** You provision it (Hetzner, DigitalOcean, ALB, nginx) and point its health check at your app's own health path. No provider APIs, no credentials, no shipnode-specific settings — SSH remains the only requirement.
  - Fleet replicas serve plain HTTP on every interface and never request a certificate — five replicas asking for one name would race Let's Encrypt. **TLS terminates at your load balancer**; pass-through TLS is out of scope. Give a fleet app a `.domain()` so the replica recognises the Host your LB forwards.
  - Blue-green is enabled automatically for fleet backends with a web port: a replica boots the new release on its idle colour, health-checks it, and only then does traffic move. `.noZeroDowntime()` is honoured — it is your outage to have.
  - A failed roll stops and reports the mixed-version fleet rather than claiming success. Updated replicas keep serving the new release; untouched ones the old; the failed replica stays on its previous release, healthy and serving.
...
- **The generated CI workflow assumed one server** — one key, one `known_hosts` entry. A host missing from `known_hosts` surfaced as an SSH failure partway through a roll, with earlier replicas already updated. The workflow now verifies every replica is a known host before deploying and prints the exact `ssh-keyscan` command to fix it.
